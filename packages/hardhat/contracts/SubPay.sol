...
contract SubPay is Ownable, ReentrancyGuard, Pausable {
    ...
    // Subscription Events
    event PlanCreated(uint256 indexed planId, address indexed merchant, uint256 amount, uint256 frequency);
    event PlanUpdated(uint256 indexed planId, bool active, uint256 amount, uint256 frequency);
    ...
    event PaymentProcessed(uint256 indexed subscriptionId, address indexed subscriber, address indexed merchant, uint256 amount);
    event PaymentFailed(uint256 indexed subscriptionId, address indexed subscriber, string reason);
    ...
    event PaymentRecorded(address indexed user, bool success, uint256 amount, address token);
    event ProviderAuthorized(address indexed provider);
    event ProviderDeauthorized(address indexed provider);
    event CreditParametersUpdated(uint256 baseScore, uint256 successBonus, uint256 failurePenalty, uint256 decayRate);
    
    // Payment Prediction Events
    event PredictionUpdated(uint256 indexed subscriptionId, uint256 likelihood, RiskLevel riskLevel);
    event OracleAuthorized(address indexed oracle);
    event OracleDeauthorized(address indexed oracle);
    event PredictionThresholdsUpdated(uint256 highSuccessThreshold, uint256 mediumSuccessThreshold, uint256 highBalanceRatio, uint256 mediumBalanceRatio);
    
    // Dispute Resolution Events
    event DisputeOpened(uint256 indexed disputeId, uint256 indexed subscriptionId, address indexed subscriber, string reason);
    event EvidenceSubmitted(uint256 indexed disputeId, address indexed submitter, string evidence);
    event DisputeResolved(uint256 indexed disputeId, Resolution resolution, address resolver, uint256 refundAmount);
    event DisputeCancelled(uint256 indexed disputeId, address canceller);
    event ArbitratorAdded(address indexed arbitrator);
    event ArbitratorRemoved(address indexed arbitrator);
    event DisputeParametersUpdated(uint256 disputeFeeRate, uint256 minDisputeAmount, uint256 resolutionTimeout);
    ...
    function subscribe(uint256 planId) external whenNotPaused nonReentrant returns (uint256 subscriptionId) {
        SubscriptionPlan storage plan = plans[planId];
        ...
        require(token.allowance(msg.sender, address(this)) >= plan.amount, "Insufficient allowance");
    ...
    function processDuePayments(uint256[] calldata subscriptionIds) external whenNotPaused nonReentrant returns (uint256 processed) {
        uint256 count = 0;
        ...
        for (uint256 i = 0; i < subscriptionIds.length; i++) {
            ...
            if (!subscription.active || subscription.nextPaymentTime > block.timestamp) {
                continue;
            }
    ...
    function getDueSubscriptions(uint256 limit) external view returns (uint256[] memory) {
    ...
    function updateCreditParameters(
        ...
        emit CreditParametersUpdated(baseScore, successBonus, failurePenalty, decayRate);
    }
    ...
    function _recordPayment(
        ...
        if (success) {
            ...
            newScore = oldScore + successBonus > maxScore ? maxScore : oldScore + successBonus;
        } else {
            // Decrease score for failed payment, floored at minScore
            newScore = oldScore < failurePenalty ? minScore : oldScore - failurePenalty;
        }
    ...
    function recordExternalPayment(
        ...
        require(msg.sender == owner() || authorizedProviders[msg.sender], "Unauthorized");
    ...
    function getPaymentHistory(address user, uint256 limit) external view returns (PaymentRecord[] memory) {
    ...
    function updatePredictionThresholds(
        ...
        require(_highSuccessThreshold > _mediumSuccessThreshold, "Invalid thresholds");
    ...
    function calculateLikelihood(uint256 subscriptionId) public view returns (uint256 likelihood, RiskLevel riskLevel) {
    ...
    function getPrediction(uint256 subscriptionId) external view returns (
        uint256 likelihood,
        uint256 lastUpdated,
    ...
    function _getHighRiskSubscriptionsIds(uint256 limit) private view returns (uint256[] memory dueSubscriptions) {
        dueSubscriptions = this.getDueSubscriptions(limit * 2);
        return dueSubscriptions;
    }
    
    function getHighRiskSubscriptions(uint256 limit) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](limit);
        uint256 count = 0;
    ...
    function openDispute(uint256 subscriptionId, string calldata reason)
        external
        nonReentrant
        returns (uint256 disputeId)
    {
        ...
        emit DisputeOpened(disputeId, subscriptionId, subscription.subscriber, reason);
        
        return disputeId;
    }
    
    function submitEvidence(uint256 disputeId, string calldata evidence)
        external
    {
    ...
    function resolveDispute(
        ...
        if (data.finalRefundAmount > 0) {
            ...
            require(
                token.transferFrom(dispute.merchant, dispute.subscriber, data.finalRefundAmount),
        ...
        emit DisputeResolved(data.disputeId, data.resolution, msg.sender, data.finalRefundAmount);
    }
    ...
    function isEligibleForAutoResolution(uint256 disputeId)
        external
        view
        returns (bool)
    {
